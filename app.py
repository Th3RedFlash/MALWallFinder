...
def clean_anime_title(title):
    ...
    text = re.sub(r'\s\(\d{4}\)$', '', text) # Corrected line 35 from previous error
...
def fetch_mal_data(username):
    ...
    if not mal_data_fetched_successfully:
        ...
        # --- CORRECTED EXCEPTION BLOCKS (Lines 110-122 area) ---
            ...
            if not anime_data_list:
                raise e # Correctly indented on its own line
        except requests.exceptions.RequestException as e:
            last_error_message = f"MAL HTML attempt - Network Error: {e}"
            print(f"  [MAL Fetch] {last_error_message}")
            if not anime_data_list:
                raise ConnectionError(last_error_message) # Correctly indented on its own line
        except Exception as e:
            last_error_message = f"MAL HTML attempt - Unexpected error: {e}"
            print(f"  [MAL Fetch] {last_error_message}"); traceback.print_exc()
            if not anime_data_list:
                raise RuntimeError(last_error_message) # Correctly indented on its own line
        # --- END CORRECTION ---
...
def group_anime(anime_data_list):
    ...
    grouped = {}
    print(f"  [GroupAnime] Starting grouping for {len(anime_data_list)} items.")
    for item_data in anime_data_list:
        title = item_data.get('title')
        if not title: print(f"  [GroupAnime] Skipping item with missing title: {item_data}"); continue
        image_url = item_data.get('image_url')
        display_title = title; search_term = title
        grouped_key = clean_anime_title(title)
        if not grouped_key: print(f"  [GroupAnime] Skipping item '{title}' because cleaned key is empty."); continue
    ...
    print(f"  [GroupAnime] Finished grouping. Result has {len(grouped)} groups.")
    return grouped
